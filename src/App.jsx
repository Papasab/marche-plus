...
const WHATSAPP_NUMBER = "22391090523";
const SUPABASE_URL    = "https://kezbscgcfobdqlitduly.supabase.co";
const SUPABASE_KEY    = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImtlemJzY2djZm9iZHFsaXRkdWx5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI0MDgxOTUsImV4cCI6MjA5Nzk4NDE5NX0.ym2Fn66HJr6HcaCy7OiXg08fFaC4GdWUudDlzHl93Xo";
...
// ─── Navbar ───────────────────────────────────────────────────────
function Navbar({ page, setPage, cartCount, user, onLogout, onProfile, favCount }) {
...
function ProductDetailPage({ product, onAdd, onBack, isFavorite, onToggleFav }) {
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 16px" }}>
      {/* Retour */}
      <button onClick={onBack} style={{ background: "none", border: "none", color: "#888", fontSize: 14, cursor: "pointer", marginBottom: 20, display: "flex", alignItems: "center", gap: 6 }}>
...
// ─── Page Panier ──────────────────────────────────────────────────
function CartPage({ cart, onRemove, onUpdateQty, goToShop, goToPayment }) {
    ...
    <div style={{ maxWidth: 680, margin: "0 auto", padding: "28px 20px" }}>
      ...
      <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #ebebeb", padding: "20px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          ...
          <div style={{ fontWeight: 800, fontSize: 24 }}>{fmt(total)}</div>
        </div>
...
// ─── Page Paiement ────────────────────────────────────────────────
function PaymentPage({ cart, onConfirm }) {
...
Merci pour votre commande! 🙏`;
  ...
  return (
    <>
      <Toast toast={toast} />
      <Navbar page={page} setPage={(p) => { setShowProfile(false); setSelectedProduct(null); setPage(p); }} cartCount={cartCount} user={user} onLogout={() => { supabase.auth.signOut(); setUser(null); }} onProfile={() => { setShowProfile(true); setSelectedProduct(null); }} favCount={favorites.length} />
      ...
      {page === "cart"    && <CartPage    cart={cart} onRemove={removeFromCart} onUpdateQty={updateQty} goToShop={() => setPage("shop")} goToPayment={() => setPage("payment")} />}
      {page === "payment" && <PaymentPage cart={cart} onConfirm={placeOrder} />}
